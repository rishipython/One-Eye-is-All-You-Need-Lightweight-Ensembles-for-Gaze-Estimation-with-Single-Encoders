...
class gazetrack_dataset(Dataset):
    def __init__(self, root, phase='train', size=(128, 128), transform=True, v=True, i=0, soupNum=3):
        self.root = root
        self.rand_amt = 10
        if(phase=='test'):
            self.files = glob(root+"*.jpg")
        else:
            self.files = sklearn.utils.shuffle(glob(root+"/images/*.jpg"), random_state=42)
            num_files = int(len(self.files)/soupNum)
            start_i = i*num_files
            end_i = min((i+1)*num_files, len(self.files))
            self.files = self.files[start_i:end_i]
        ...
        if(v):
            if (phase=='test'):
                ...
                print("Num files for " + phase + f" (i = {i}) = " + str(len(self.files)))
