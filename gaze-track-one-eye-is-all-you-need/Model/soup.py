...
def uniform_soup(model, path, device = "cpu", by_name = False):
    ...
    for i, model_path in enumerate(path):
        weight = torch.load(model_path, map_location = device)['state_dict']
...
def greedy_soup(model, path, data, metric, device = "cpu", update_greedy = False, compare = np.less_equal, by_name = False, digits = 4, verbose = True, y_true = "y_true"):
    try:
        import torch
    except:
        print("If you want to use 'Model Soup for Torch', please install 'torch'")
            ...
            try:
                ...
                with torch.no_grad():
                    ...
                    if isinstance(iter_data, dict):
                        ...
                        y = [d.to(device) if isinstance(d, torch.Tensor) else d for d in y]
                        d_cnt = len(y[0])
                    metric_val = np.array(metric(np.array([pred.cpu().detach().numpy() for pred in logits])[0], np.array([yi.cpu().detach().numpy() for yi in y])))
